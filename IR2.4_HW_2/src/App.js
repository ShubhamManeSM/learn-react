...
const Gym = ({gyms, name}) => {
   const gymData = gyms.find(gym => gym.name === name) 
   return(
    <div>
      <h2>Gym Data</h2>
...
const Travel = ({travels, destination}) => {
  const travelDetails = travels.find(travel => travel.destination === destination)
  return(
    <div>
...
const Laptop = ({laptops, brand}) => {
  const laptopData = laptops.find(laptop => laptop.brand === brand)
  return(
    <div>
      <h2>Laptop Data</h2>
...
export default function App() {
  ...
  return (
    <div>
      ...
      <hr />
      <Gym gyms={gyms} name="Gym 2" />
      <hr />
      <Travel travels={travels} destination="Travel 3" />
      <hr />
      <Laptop laptops={laptops} brand="Laptop 3" />
      <hr />
      <Recipe recipes={recipes} dish="Recipe 2" />
      <hr />
      <Blog blogs={blogPosts} id={3} />
    </div>
  );
}
